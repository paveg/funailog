...
メインマシンはMacBook Air M3です。メモリ16GBで開発には十分ですし、軽さは正義だと思っています。
...
起動時間は約50msです。powerlevel10kのinstant promptとzinit turbo modeの組み合わせで実現しています。
...
# ベンチマーク
$ time zsh -i -c exit
zsh -i -c exit  0.03s user 0.02s system 92% cpu 0.054 total
```
...
**Tailscale** は自宅のサーバーにアクセスするために使っています。VPNの設定が簡単で、どこからでも安全に接続できます。
