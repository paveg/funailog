...
- Zod はメソッドチェーンによる直感的なAPIで、全機能がバンドルされる
- Valibot は関数合成ベースのモジュラー設計で、使用する機能のみがtree-shakingで抽出される
...
- フレームワークは Hono
- バリデーションは Zod から Valibot へ移行
- Hono ミドルウェアは @hono/zod-validator から @hono/valibot-validator へ移行
