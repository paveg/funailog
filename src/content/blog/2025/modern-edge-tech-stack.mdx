...
妥協のないUXを実現するために、技術選定においても「Zero Friction」を徹底しました。
キーワードは **"Speed is a Feature"**（速度は機能である）です。

## TL;DR

- Edge First の方針として、Cloudflare Workers + D1 で「物理的な距離」を最小化
- Optimistic UI を TanStack Query で実装し、「通信待ち時間」を体感ゼロにした
- Offline Ready により、電波の悪いジムでも使える「完全オフライン対応」を実現
- DX 面では Hono + Expo を採用し、個人開発でも「爆速」で機能提供できる
...
ジムという環境は過酷です。地下で電波が悪かったり、インターバル中の限られた時間で操作することになります。
...
- Webのスピード感として、`git push` で即デプロイ、即ユーザーの手元へ反映できる
- Native の品質も担保される。React Native で書いているため、いつでもiOS/Androidアプリとしてビルドできる

Tamaguiを採用したことで、WebとNativeでスタイルを99%共有できています。
「Webで検証し、完成度が高まったらStoreへ」という、個人開発に向いたロードマップを歩めています。
