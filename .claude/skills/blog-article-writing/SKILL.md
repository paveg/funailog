...
frontmatter を先に確定する。
ファイル配置・frontmatter・シリーズ slug 整合・「想定読者」の位置などの規約は
`src/content/blog/CLAUDE.md` を参照する（スキーマの原典は `src/schemas/blog-collection.ts`）。
tags は 3-7 個、製品名/技術名/カテゴリ関連の意味あるキーワード。
emoji は記事内容と意味的に対応する 1 文字（例: キーボードなら ⌨️、コンテナなら 📦）。
